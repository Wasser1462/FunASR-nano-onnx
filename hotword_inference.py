...
from hotwords.hotword_corrector import PhonemeCorrector, RectificationRAG


def append_rectify_entry(file_path: str, wrong: str, right: str):
    ...
    try:
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    for block in content.split('---'):
                        ...
                        if len(lines) >= 2:
                            w, r = lines[0], lines[1]
                            existing_pairs.add((w, r))
            except Exception:
                pass  
        
        if (wrong, right) in existing_pairs:
        ...
        print(f"[Rectify] failed to append history: {e}")
...
def get_args():
    ...
    # Hotword correction arguments
    p.add_argument("--hotword-file", type=str, default=None, help="Path to hotword file (one per line)")
    p.add_argument("--rectify-file", type=str, default=None, help="Path to correction history file")
    p.add_argument("--hotword-threshold", type=float, default=0.7, help="Hotword matching threshold")
    p.add_argument("--similar-threshold", type=float, default=None, help="Similar word threshold (default: hotword_threshold - 0.2)")
    p.add_argument("--rectify-threshold", type=float, default=0.5, help="Correction history matching threshold")
    p.add_argument("--disable-correction", action="store_true", help="Disable hotword correction")
...
def main():
    ...
    if generated:
        ...
        # Apply hotword correction if enabled
        if not args.disable_correction:
            ...
            # Load hotwords and correction history
            if args.hotword_file:
                count = corrector.load_hotwords_file(args.hotword_file)
                if count > 0:
                    print(f"[Hotword] Loaded {count} hotwords from {args.hotword_file}")
            
            if args.rectify_file:
                rectifier.load_rectify_file(args.rectify_file)
                print(f"[Rectify] Loaded correction history from {args.rectify_file}")
            
            # Apply correction: two-stage process
            # Stage 1: Hotword correction
            # Stage 2: History-based correction on hotword-corrected text
            if corrector.hotwords or rectifier.records:
                # Stage 1: Apply hotword correction
                correction_result = corrector.correct(out_text)
                stage1_text = correction_result.text
                hotword_applied = stage1_text != out_text
                
                # Stage 2: Apply history-based correction on stage1 result
                stage2_text, history_matches = rectifier.apply_corrections(stage1_text, top_k=1)
                history_applied = stage2_text != stage1_text
                
                # Debug: show history search results if not applied
                if not history_applied and rectifier.records:
                    debug_matches = rectifier.search(stage1_text, top_k=3)
                    if debug_matches:
                        print(f"[History Debug] Found {len(debug_matches)} potential matches (not applied):")
                        for wrong, right, score in debug_matches:
                            print(f"  '{wrong}' => '{right}' (score: {score:.3f})")
                
                # Use final corrected text if any correction was applied
                if hotword_applied or history_applied:
                    final_text = stage2_text
                    print(f"[Original] {out_text}")
                    if hotword_applied:
                        print(f"[Hotword Corrected] {stage1_text}")
                        if correction_result.matches:
                            print(f"[Hotword Matches] {len(correction_result.matches)}")
                            for wrong, right, score in correction_result.matches:
                                print(f"  {wrong} => {right} (score: {score:.3f})")
                    if history_applied:
                        print(f"[History Corrected] {final_text}")
                        if history_matches:
                            print(f"[History Matches] {len(history_matches)}")
                            for wrong, right, score in history_matches:
                                print(f"  {wrong} => {right} (score: {score:.3f})")
                    else:
                        print(f"[Final] {final_text}")
                    
                    # Persist to rectify file for future runs (only if final result differs from original)
                    if args.rectify_file and final_text != out_text:
                        append_rectify_entry(args.rectify_file, out_text, final_text)
                    out_text = final_text
                else:
                    # Show all potential matches even if not applied
                    all_history_matches = rectifier.search(out_text)
                    if correction_result.matches or all_history_matches:
                        print(f"[Original] {out_text}")
                        if correction_result.matches:
                            print(f"[Hotword Matches (not applied)] {len(correction_result.matches)}")
                            for wrong, right, score in correction_result.matches:
                                print(f"  {wrong} => {right} (score: {score:.3f})")
                        if all_history_matches:
                            print(f"[History Matches (not applied)] {len(all_history_matches)}")
                            for wrong, right, score in all_history_matches:
                                print(f"  {wrong} => {right} (score: {score:.3f})")
                    else:
                        print(out_text)
            else:
